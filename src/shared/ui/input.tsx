...
function Input({ className, type, suffix, ...props }: InputProps) {
  return (
    <div className="relative w-full">
      ...
      />
      {suffix && (
        <div className="absolute right-2.5 top-1/2 -translate-y-1/2 text-sm text-muted-foreground pointer-events-none">
          {suffix}
        </div>
      )}
    </div>
